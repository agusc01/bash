...
alias ethernetinterface='nmcli device status | grep ethernet | cut -d " " -f1'
...
alias wifiinterface='nmcli device status | grep wifi | cut -d" " -f1'
alias wifiscan='sudo ip link set wlo1 up && sudo iwlist wlo1 scan | grep SSID | cut -d: -f2 | tr -d " \"" | sort | uniq'
alias wifioff='sudo ifdown $(wifiinterface) && refreshdbar && sudo rfkill block wlan'
alias wifion='sudo kill -9 $(pgrep wpa_supplicant) || true && sudo rfkill unblock wlan && sleep 1 && sudo ifup $(wifiinterface) && refreshdbar'
